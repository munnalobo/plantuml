...
public class LogEntryAspect {
  ...
  private void saveLog(JoinPoint joinPoint, Instant executionTime, boolean returningCall) {
    List<String> classExclusionList = List.of("Aspect", "Signature", "$", "set", "get",
        "aroundBody", "<generated>");
    ...
    if (!aspect.isEmpty()) {
      ...
      if (!callingMethodName.equals(currentMethodName)) {
        log.info("current method: {}, Calling class: {}, Calling method: {}", currentMethodName,
        ...
        if (returningCall) {
          builder = CustomLog.builder().currentMethodName(callingMethodName)
              .currentClassName(callingClassName).callingClassName(currentClassName)
              .callingMethodName(currentMethodName);
        } else {
          builder = CustomLog.builder().currentMethodName(currentMethodName)
              .currentClassName(currentClassName).callingClassName(callingClassName)
              .callingMethodName(callingMethodName);
        }
        build = builder.executionTime(executionTime).currentSpan(key).returnCall(returningCall)
            .build();
  ...
  public Object aspectForLogging(ProceedingJoinPoint joinPoint) throws Throwable {
    try {
      if (!joinPoint.getSignature().getDeclaringType().getName().contains("$")
          && !joinPoint.getSignature().getDeclaringType().getName().contains("models")
          && !joinPoint.getSignature().getName().contains("$")) {
        if (Objects.isNull(tracer) && Objects.nonNull(SomeService.getInstance()) && Objects.nonNull(
            SomeService.getInstance().getTracer())) {
          this.tracer = SomeService.getInstance().getTracer();
        }

        log.info("Before: " + joinPoint.getSignature().getName());
        if (Objects.nonNull(tracer) && Objects.nonNull(tracer.currentSpan()) && Objects.nonNull(
            tracer.currentSpan().context()) && Objects.nonNull(
  ...
  public String generatePumlDiagram() {
    ...
    Set<String> collect = SomeService.getInstance().getSpanAndLogsMap().values().iterator().next()
        .stream().sorted(Comparator.comparing(CustomLog::getExecutionTime))
        .flatMap(x -> Stream.of(x.getCurrentClassName(), x.getCallingClassName()))
    ...
    next.forEach(x -> {
      ...
      sb.append(x.getCallingClassName()).append(str).append(x.getCurrentClassName()).append(" ")
          .append("randomHex()").append(" : ").append(x.getCallingMethodName()).append(" - ")
          .append(x.getCurrentMethodName()).append("\n");
      strings.add(sb.toString());
    });

    ChatGPTVersion chatGPTVersion = new ChatGPTVersion();
    LinkedList<PatternMetaData> patternMetaDataLinkedList = chatGPTVersion.extracted(strings);

    patternMetaDataLinkedList.forEach(patternMetaData -> {
      if (patternMetaData.repeatCount > 1) {
        sb.append("loop ").append(patternMetaData.getRepeatCount());
        patternMetaData.getPattern().stream().map(x -> x.replace("randomHex()", randomHex())).forEach(x -> {
          sb.append(x);
        });
        sb.append("end");
      } else {
